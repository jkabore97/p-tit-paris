    ...
    <header className={`sticky top-0 z-50 transition-all duration-500 ${scrolled || open ? "glass shadow-[0_10px_40px_-24px_rgba(74,44,28,.5)]" : "bg-transparent"}`}>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3">
        <Link href="/" className="flex items-center gap-3">
          <Logo className="h-10 w-10 text-candy transition-transform duration-500 hover:rotate-12" />
          <span className="font-display text-2xl font-bold tracking-tight text-ink">{site.name}</span>
        </Link>
        <nav className="hidden items-center gap-7 md:flex">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className={`relative text-sm font-medium uppercase tracking-[0.18em] transition-colors after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:rounded-full after:bg-candy after:transition-all ${
                path.startsWith(l.href) ? "text-candy after:w-full" : "text-ink/70 after:w-0 hover:text-ink hover:after:w-full"
              }`}
            >
              {l.label}
            </Link>
          ...
          <Link
            href="/commander"
            className="btn-shine relative rounded-full bg-candy px-5 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white shadow-lg shadow-candy/30 transition hover:-translate-y-0.5"
          >
            Ma commande
            {count > 0 && (
